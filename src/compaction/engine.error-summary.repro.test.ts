...
function buildStore(sessionId: string): MemoryStore {
  const store = new MemoryStore();
  store.openConversation(sessionId, '/tmp/project');
...
describe('runCompaction regression suite for unusable summaries', () => {
  it('keeps context unchanged when complete() resolves with stopReason="error"', async () => {
    const store = buildStore('sess_error_repro');
    ...
    assert.strictEqual(
      result.actionTaken,
      false,
      'Expected actionTaken false when complete() resolves with stopReason="error"',
    );
  ...
  it('keeps context unchanged when complete() returns no text parts', async () => {
    const store = buildStore('sess_notext_repro');
    ...
    assert.strictEqual(
      result.actionTaken,
      false,
      'Expected actionTaken false when complete() returns no text parts',
    );
    assert.strictEqual(result.summariesCreated, 0);
    assert.deepStrictEqual(store.getContextItems(), [
      { kind: 'message', messageId: 'm0' },
