...
describe('runCompaction summary_error_response regression', () => {
  it('records exactly summary_error_response with no extra no-op noise', async () => {
    const store = new MemoryStore();
    store.openConversation('sess_error_response_regression', '/tmp/project');
    ...
    // Summarizer always throws error_response — both leaf and condensed paths
    ...
    assert.deepStrictEqual(result.noOpReasons, ['summary_error_response']);
    assert.ok(
